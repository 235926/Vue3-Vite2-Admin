...
<script setup>
import Contextmenu from './contextmenu.vue' // 子组件
import { onBeforeRouteUpdate } from 'vue-router' // 路由
import { isObjectValueEqual, isMobile } from '@/utils/tools.js' // 工具方法
...
const onCurrentContextmenuClick = async (item) => {
    ...
    switch (item.contextMenuClickId) {
        ...
        case 1:
            // 关闭当前
            closeCurrentTagsView(layoutConfig.value.isShareTagsView ? path : url)
            break
        case 2:
            // 关闭其它
            if (meta.isDynamic) await router.push({ name, params })
        ...
        case 4:
            // 开启当前页面全屏
            openCurrenFullscreen(layoutConfig.value.isShareTagsView ? path : url)
            break
    }
}

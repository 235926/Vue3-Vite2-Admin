...
/**
 * 3、一维数组处理成多级嵌套数组（只保留二级：也就是二级以上全部处理成只有二级，keep-alive 支持二级缓存）
...
/**
 * 5、获取当前用户权限标识去比对路由表（未处理成多级嵌套路由）
 * @description 这里主要用于动态路由的添加，router.addRoute
 * @link 参考：https://next.router.vuejs.org/zh/api/#addroute
 * @param chil dynamicRoutes（@/router/modules/dynamicRoutes.js）第一个顶级 children 的下路由集合
...
export function setCacheTagsViewRoutes() {
    ...
    let rolesRoutes = setFilterHasRolesMenu(dynamicRoutes, store.getters.userInfo.roles)

<template>
    <div class="filtering-more">
        <el-alert title="过滤条件为多选" type="success" :closable="false" class="mb20"></el-alert>
        ...
        <div v-for="(val, key) in state.filtering" :key="key" :ref="(el) => { if (el) dlRefs[key] = el }"
            ...
            <div class="filtering-list-item" :style="{ height: val.isMore ? 'auto' : '54px' }">
                <el-tag :effect="v.active ? 'dark' : 'light'" v-for="(v, k) in val.children" :key="k"
                    @click="onSelItem(v)">{{ v.label }}</el-tag>
...
const initActive = () => {
    state.filtering.map(item => {
        state.setFiltering[item.value] = []
        item.children.map(chil => {
            if (chil.active) {
                state.setFiltering[item.value].push(chil.value)
            }
        })
    })
}
...
const getFuncFiltering = () => {
    ...
    for (let i in state.setFiltering) {
        let obj = {
            [i]: state.setFiltering[i]
        }

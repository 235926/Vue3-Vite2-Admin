<template>
    <div class="page-container">
        <el-card shadow="hover" header="表单表格验证">
                ...
                <el-table :data="state.tableData.data" border class="module-table-uncollected">
                    <el-table-column v-for="(item, index) in state.tableData.header" :key="index" show-overflow-tooltip
                        ...
                        <template v-slot="scope">
                            <el-form-item :prop="`data.${scope.$index}.${item.prop}`" :rules="[{
                                ...
                                <el-select v-if="item.type === 'select'" v-model="scope.row[item.prop]"
                                    placeholder="请选择">
                                    <el-option v-for="sel in state.tableData.option" :key="sel.id" :label="sel.label"
                                        :value="sel.value" />
                                </el-select>
                                <el-date-picker v-else-if="item.type === 'date'" v-model="scope.row[item.prop]"
            ...
            <el-row class="flex mt20">
                <div class="flex-margin">
                    <el-button type="success" @click="onValidate">表格验证</el-button>
...
<script setup name="funcTableRules">
...
// 表格验证
const onValidate = () => {
    if (state.tableData.data.length <= 0) return proxy.$message.warning("请先点击增加一行")
    tableRulesRef.value.validate((valid) => {
        if (!valid) return proxy.$message.warning("表格项必填未填")
        proxy.$message.success("全部验证通过")
    })
}
